/*******************************************************************************************
 ...
 Therefore, the following relationships may hold:
  ...
  The functions drawTree() and drawTreeHelper() are not parts of splay tree. I have
  ...
  of now. When you are done with the core concepts (functions) of splay tree,
...
//# define DEBUG // Comment out this or # undef DEBUG if you do not want verbose output
...
# define SIZE1 20   // Increase this value if you have a tree with higher depth
# define SIZE2 255  // Do not change this value
...
/** I used this function to debug the code. Though it is useless now,
...
Tree insertIntoSplayTree(Tree tree, int key) {
    ...
    if (tree == NULL) {
        tree = insertNodeIntoBST(tree, newNode);
        printf("After inserting %d in BST:\n", key);
        drawTree(tree);
    } else {
        tree = insertNodeIntoBST(tree, newNode);
        printf("After inserting %d in BST:\n", key);
